...
_KB = {
    ...
    'shadow_endpoint': ('API9:2023 Improper Inventory Management',
             "An endpoint present in traffic is absent from the API spec (undocumented/shadow).",
             "Inventory and document every endpoint; retire shadow/debug routes in prod."),
...
def _narrative_html(chains, recommendations, escalations, e) -> str:
    """Section récap « narratif d'attaque » : escalades (Phase 4), chaînes
    d'exploitation et actions recommandées. Vide -> chaîne vide (rien rendu)."""
    blocks = []
    if escalations:
        items = ''.join(f'<li>{e(str(x.get("name", "")))}</li>' for x in escalations)
        blocks.append(f'<div class="narr esc"><div class="label">Escalades '
                      f'({len(escalations)})</div><ul class="escs">{items}</ul></div>')
    if chains:
        cards = []
        for c in chains:
            conf = c.get('confidence', 0)
            steps = ''.join(f'<li>{e(str(s))}</li>' for s in c.get('steps', []))
            why = e(str(c.get('rationale', '')))
            src = e(str(c.get('source', '')))
            cards.append(
                f'<article class="chain"><div class="chead">'
                f'<span class="conf">conf {conf}</span>'
                f'<h3>{e(str(c.get("title", "")))}</h3>'
                f'<span class="csrc">{src}</span></div>'
                f'<ol class="steps">{steps}</ol>'
                f'{f"<p class=why>{why}</p>" if why else ""}</article>')
        blocks.append(f'<div class="narr"><div class="label">Chaînes '
                      f'd\'exploitation ({len(chains)})</div>{"".join(cards)}</div>')
    if recommendations:
        items = ''.join(
            f'<li><b>{e(str(r.get("trigger", "")))}</b> → {e(str(r.get("action", "")))}</li>'
            for r in recommendations)
        blocks.append(f'<div class="narr"><div class="label">Actions recommandées '
                      f'({len(recommendations)})</div><ul class="recos">{items}</ul></div>')
    if not blocks:
        return ''
    return '<section class="narrative"><h2>Narratif d\'attaque</h2>' + ''.join(blocks) + '</section>'


def render_html(findings: List[Finding], meta: Optional[Dict] = None,
                chains=None, recommendations=None, escalations=None) -> str:
    meta = meta or {}
    counts = {s: sum(1 for f in findings if f.severity == s)
              for s in ('Critical', 'High', 'Medium', 'Low', 'Info')}
    e = _html.escape
    narrative = _narrative_html(chains, recommendations, escalations, e)
    cards = []
    for f in findings:
...
.fix{{margin:0}}
.narrative{{margin:0 0 30px}} .narrative h2{{font-size:1.15rem;margin:0 0 12px}}
.narr{{margin-bottom:16px}}
.chain{{border:1px solid var(--line);border-left:5px solid var(--accent);border-radius:12px;background:var(--surface);padding:14px 16px;margin-bottom:10px}}
.conf{{font-family:ui-monospace,monospace;font-size:.68rem;color:#fff;background:var(--accent);padding:2px 8px;border-radius:999px}}
.csrc{{font-family:ui-monospace,monospace;font-size:.68rem;color:var(--muted)}}
.steps{{margin:10px 0 8px;padding-left:22px}} .steps li{{margin:3px 0}}
.why{{margin:0;color:var(--muted);font-size:.9rem}}
.escs,.recos{{margin:0;padding-left:20px}} .escs li,.recos li{{margin:3px 0}}
.esc .escs li{{color:var(--accent);font-weight:600}}
